...
column_alias
 ...
 | STRING_LITERAL
 ;
 string_rules: // problem with ""
 //( (NUMERIC_LITERAL)* |(Chracter)*|(NUMERIC_LITERAL*Chracter*) )
  '"'RANDOM_NAME'"'
 ;
 char_rule:// problem with digit  + ''
DIGIT
 ;
...
K_PRINT OPEN_PAR  indisde_the_print  CLOSE_PAR SCOL
 ;
 indisde_the_print:
((indisde_the_print_part1? IDENTIFIER indisde_the_print_part2?)|(RANDOM_NAME|call_function))*
 ;

indisde_the_print_part1:
((RANDOM_NAME|call_function) PLUS)
;

indisde_the_print_part2:
 (PLUS (RANDOM_NAME|call_function))
;
...
K_FOR
'('
 (K_VAR use_random_name '=' NUMERIC_LITERAL )? SCOL( any_name comparative_processes NUMERIC_LITERAL )?
  SCOL ( increase_decrease any_name increase_decrease )?
')'
'{'
...
arithematic_rule  :
operand_of_arithematic_rule comparative_processes operand_of_arithematic_rule
;
s:
arithematic_rule* '>' arithematic_rule*
;
...
keyValuePair
    : RANDOM_NAME ':' (primitive | jsonObject)
    ;
